...
Ltac zsimpl_strat := try (rewrite_strat (repeat (topdown (hints zsimpl)))).
Ltac zcongruence := repeat red; intros; zsimpl; congruence.
Ltac zify_strat := unfold eq, lt, le in *; zsimpl_strat.
...
Proof.
 intros a b. zify_strat. intros Hb.
 destruct [b]; reflexivity || discriminate.
Qed.

Lemma pow_pow_N : forall a b, 0<=b -> a^b == pow_N a (Z.to_N (to_Z b)).
Proof.
 intros a b. zify_strat. intros Hb. now rewrite spec_pow_N, Z2N.id.
Qed.
...
Proof.
 intros n. zify_strat. apply Z.sqrt_neg.
Qed.
...
Proof.
 intros n. zify_strat. apply Z.log2_spec.
Qed.
...
Proof.
intros a b. zify_strat. intros. apply Z_mod_lt; auto with zarith.
Qed.

Theorem mod_neg_bound :
 forall a b, b < 0 -> b < modulo a b /\ modulo a b <= 0.
Proof.
intros a b. zify_strat. intros. apply Z_mod_neg; auto with zarith.
Qed.
...
Proof.
intros a b. zify_strat. apply Z.quot_rem.
Qed.

Theorem rem_bound_pos :
 forall a b, 0<=a -> 0<b -> 0 <= rem a b /\ rem a b < b.
Proof.
intros a b. zify_strat. apply Z.rem_bound_pos.
Qed.

Theorem rem_opp_l : forall a b, ~b==0 -> rem (-a) b == -(rem a b).
Proof.
intros a b. zify_strat. apply Z.rem_opp_l.
Qed.

Theorem rem_opp_r : forall a b, ~b==0 -> rem a (-b) == rem a b.
Proof.
intros a b. zify_strat. apply Z.rem_opp_r.
Qed.
...
Proof.
 ...
 - intros (z,H). exists (of_Z z). now zify_strat.
Qed.
...
Proof.
 intros a n m. zify_strat. intros Hn H.
